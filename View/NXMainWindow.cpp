...

{
    ...
    //创建快捷键
    QShortcut* c=new QShortcut(QKeySequence(tr("F12")),this);
    connect(c,SIGNAL(activated()),this,SLOT(on_slot_fullscreen()));
   // this->showMaximized();
...
void NXMainWindow::addDockWidget(Qt::DockWidgetArea area, NXDockWidget* dockWidget, Qt::Orientation orientation)
{
	...
	if(m_menuWindows == nullptr)
	{
        ...
        m_menuWindows = menuBar()->addMenu(Tr.GetTr("FrameMenu"));
...
void NXMainWindow::on_slot_fullscreen()
{
    if(isFullScreen())
    {
        showNormal();
